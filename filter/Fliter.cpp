...
namespace OpenIP{
    ...
    std::shared_ptr<PixelMap> Fliter::gray(){
        std::vector<std::vector<std::shared_ptr<Pixel>>> tmp = this->pixelMap->getPixelMap();
        for (int i = 0; i < this->pixelMap->getWidth(); i++) {
            for (int j = 0; j < this->pixelMap->getHeight(); j++) {
                    float gray = tmp[i][j]->getColor()->R() * 0.03f +
                                  tmp[i][j]->getColor()->G() * 0.59f +
                                  tmp[i][j]->getColor()->B() * 0.11f;
                    pixelMap->changeColor(i,j,std::make_shared<ColorRGB>(gray*255,gray*255,gray*255));
            }
        }
        return this->getPixelMap();
    }
